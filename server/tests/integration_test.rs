...
async fn test_single_client_login() {
    let server_addr = start_test_server().await.expect("Failed to start server");
...
async fn test_voice_channel_join() {
    ...
    let join_voice_pkt = TcpPacket::new(PacketTypeId::JoinVoiceChannel, encode_username("alice"));
    ...
    let join_voice_pkt = TcpPacket::new(PacketTypeId::JoinVoiceChannel, encode_username("bob"));
